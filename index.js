...
const {
  ...
  sendContactEmail,
} = require('./backend/contact-service');
...
app.get('/health', (_req, res) => {
  res.json({ status: 'OK', service: 'Cendoj MCP Server', website: 'NEXORA Digital Contact', version: '1.2.0' });
});
...
app.get('/api/contact/submissions', (req, res) => {
  ...
  return res.status(200).json({ ok: true, total: submissions.length, data: submissions });
});
...
app.post('/api/contact', async (req, res) => {
  ...
  try {
    ...
    const lead = {
      ...
      emailDelivery: emailResult.mocked ? 'mocked' : 'sent',
      emailId: emailResult.id,
    };

    submissions.unshift(lead);
    ...
    return res.status(502).json({
      ok: false,
      error: error instanceof Error ? error.message : 'No se pudo procesar el envío',
    });
  }
});
